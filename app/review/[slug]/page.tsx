  ...
  return (
    <article className="max-w-3xl mx-auto px-4 py-8">
      ...
      <section id="reviews" className="scroll-mt-20 mb-12">
        ...
        <figure className="mt-8">
          <img src={r.subImage} alt={r.subAlt} width={1200} height={630}
            className="w-full rounded-2xl border border-border object-cover aspect-[1200/630]" />
          <figcaption className="mt-2 text-xs text-text-muted text-center">床のメンテナンスは作業内容（ワックスがけ・フロアコーティング・剥離洗浄等）や床材によって費用が異なります。</figcaption>
        </figure>
      </section>
      ...
      {r.officialUrl ? (
        ...
        <div className="rounded-2xl border border-border bg-surface-alt p-6 text-center">
          <p className="text-sm text-text-secondary">現在、公式サイトの稼働が確認できないため、リンクは掲載していません。最新の提供状況はご自身でもご確認ください。</p>
          <p className="mt-3 text-xs text-text-muted">掲載情報は{r.updatedAt}時点のものです。</p>
        </div>
      )}

      <div className="mt-8 flex flex-wrap gap-4 text-sm">
        <Link href="/review/" className="text-primary hover:underline">他の業者の口コミ・評判を見る</Link>
        <Link href="/ranking/" className="text-primary hover:underline">床メンテナンス業者ランキングを見る</Link>
      </div>
    </article>
  );
}
